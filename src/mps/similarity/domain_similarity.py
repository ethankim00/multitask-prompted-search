...
class UnsupervisedDomainFeatureExtractor(DomainFeatureExtractor):
    ...
    def _extract_features(
        self, domain: str, data_dir: str = "./data/"
    ) -> np.ndarray:
...
class DomainSimilarity:
    ...
    def _calculate_similarities(
        ...
        if self.method == "average":
            return cosine_similarity(np.mean(emb1, axis=0).reshape(1, -1), np.mean(emb2, axis=0).reshape(1, -1))
    ...
    def return_domain_similarities(
        ...
        domain_similarites = [(self.domains[i], score) for i, score in enumerate(self.similarities[self.domains.index(domain)]) if self.domains[i] != domain]
        if k is not None:
            print(k)
