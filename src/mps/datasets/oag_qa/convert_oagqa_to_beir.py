...
def save_jsonl(
    output_dir: str = "./data", filename: str = "corpus.jsonl", data: List[Dict] = []
) -> None:
...
def extract_qrels(
    oag_query_df: pd.DataFrame, oag_corpus_df: pd.DataFrame
) -> pd.DataFrame:
    oag_query_df["positive_ids"] = oag_query_df[1].apply(
        lambda x: mine_positive_document_ids(eval(x), oag_corpus_df)
    )
...
def extract_queries(oag_query_df: pd.DataFrame) -> List[Dict]:
    return [
        {"_id": str(idx), "text": text, "metadata": {}}
        for idx, text in zip(oag_query_df.index, oag_query_df[0])
    ]


def train_test_val_split(
    df: pd.DataFrame, train_split: float = 0.8, dev_split: float = 0.1
):
...
class OAGBeirConverter:
    ...
    def _load_oag_data_raw(
        self, topic_name: str, file_type: str, data_dir: Optional[str] = None
    ...
    def _load_all_files(self, topic_name: str) -> Dict:
        result = {
            file_type: self._load_oag_data_raw(topic_name, file_type)
            for file_type in ["corpus", "queries", "qrels"]
        }
        return result
    ...
    def convert_oag_beir_corpus(oag_corpus: pd.DataFrame) -> List[Dict]:
        converted_corpus = [
            {"_id": str(_id), "title": "", "text": str(title) + " " + str(text)}
            for _id, text, title in zip(oag_corpus[0], oag_corpus[1], oag_corpus[2])
        ]
        return converted_corpus

    def convert(
        self, dataset: str, output_dir: str = "./data/", data_dir: Optional[str] = None
    ) -> str:
        data_dir = data_dir if data_dir else self.data_dir
        ...
        beir_qrels.to_csv(qrels_folder.joinpath("train.tsv"), sep="\t", index=False)
        beir_qrels.to_csv(qrels_folder.joinpath("dev.tsv"), sep="\t", index=False)
        beir_qrels.to_csv(qrels_folder.joinpath("test.tsv"), sep="\t", index=False)
        return dataset_folder
