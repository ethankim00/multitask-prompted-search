...
from src.mps.models import (
    SoftPromptModelArguments,
    load_soft_prompt_model,
    DeltaModelSentenceTransformer,
)
...
class Trainer:
    def fit(
        ...
        for loss_model in loss_models:
            ...
            scheduler_obj = model._get_scheduler(
                optimizer,
                scheduler=scheduler,
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            for _ in trange(
                ...
                for train_idx in range(num_train_objectives):
                    ...
                    features = list(
                        map(
                            lambda batch: batch_to_device(batch, model._target_device),
                            features,
                        )
                    )
            ...
            if wandb_log:
                ...
                soft_prompt_token_nearest_neighbors = (
                    model.get_top_similar_vocab_words()
                )
...
def load_ir_evaluator(
    ...
    corpus = {
        idx: corpus[idx].get("title") + " " + corpus[idx].get("text") for idx in corpus
    }
    ...
    if max_corpus_size:
        ...
        if len(corpus_ids) > max_corpus_size:
            raise ValueError(
                "Your maximum corpus size should atleast contain {} corpus ids".format(
                    len(corpus_ids)
                )
            )
    ...
    logger.info(
        "{} set contains {} documents and {} queries".format(
            name, len(corpus), len(queries)
        )
    )
    return InformationRetrievalEvaluator(
        queries, corpus, rel_docs, name=name, show_progress_bar=True
    )
...
def train(
    ...
    try:
        ...
        ir_evaluator = load_ir_evaluator(
            dev_corpus, dev_queries, dev_qrels, max_corpus_size=max_corpus_size
        )
    except:
