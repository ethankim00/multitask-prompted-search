...
parser.add_argument("--doc_template", type=str, default="<title> <text>")
parser.add_argument("--query_max_len", type=int, default=128)
parser.add_argument("--doc_max_len", type=int, default=256)
parser.add_argument(
    "--tokenizer", type=str, required=False, default="bert-base-uncased"
)
...
def process_item(item):
    ...
    query = tokenizer.encode(
        fill_template(args.query_template, item, ["question"]),
...
for chunk in divide_chunks(data, 10000):
    with Pool(args.num_workers) as p:
        groups = list(
            tqdm(
                p.imap(process_item, chunk, chunksize=args.mp_chunk_size),
                total=len(chunk),
            )
        )
